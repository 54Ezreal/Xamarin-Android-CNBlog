...
{
	public class EssencePageFragement : BaseFragment, OnRefreshListener
    { 
		...
		public View comAdapter_OnGetView(int position, View convertView, ViewGroup parent, Article item, ViewHolder viewHolder)
		{
            ...
            if (imgView != null)
            {
                if (item.Avatar.Contains("png") || item.Avatar.Contains("jpg"))
        ...
        public void onLoadMore(PullToRefreshLayout pullToRefreshLayout)
        {
			BaseService.ExeRequest(async () =>
			{
				...
				var moreArticles = await BlogService.GetPickedArticles(CommonHelper.token, pageIndex, pageSize);
